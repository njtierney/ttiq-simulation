---
title: "Untitled Draft"
author: "Report Author"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
## target knits Rmds in their own session, so load libraries here.
## source("here::here(packages.R"))
```

```{r load-targets, include=FALSE}
# tar_load(c(target_1, target_2, target_3))

# # If your chunk output is shown in-line, then you'll need to wrap tar_load()
# # like so:
# 
withr::with_dir(here::here(), {
  tar_load(
    c(
      scenario_df_run
      )
    )
})
# 
# # This is not needed when using tar_make() to render the document.
```

## Analysis

```{r show-trace-run}
scenario_df_run %>% 
  select(r_after_contact_tracing,
         tp_multiplier,
         r_start)
```

```{r}
scenario_df_run %>% 
  mutate(sims = future_map(time_to_isolation_sims, c)) %>% 
  unnest(cols = sims) %>% 
  ggplot(aes(x = sims)) +
    geom_histogram(
      bins = 50,
      colour = "white",
      fill = "grey"
    ) +
  facet_wrap(~r_start,
             labeller = label_glue("R start = {r_start}")) +
    labs(
      x = "Time from infection to isolation"
    ) +
    theme_minimal()
```

```{r}
scenario_df_run %>% 
  select(r_after_contact_tracing,
         tp_multiplier,
         r_start,
         mu) %>% 
  ggplot(aes(x = mu,
             y = r_after_contact_tracing,
             colour = r_start)) +
  geom_point()
```


```{r}
ggplot(data = scenario_df_run,
       aes(x = mu,
           y = tp_multiplier,
           colour = r_start)) +
  geom_point()
```


## Reproducibility

<details><summary>Reproducibility receipt</summary>

```{r}
## datetime
Sys.time()

## repository
if(requireNamespace('git2r', quietly = TRUE)) {
  git2r::repository()
} else {
  c(
    system2("git", args = c("log", "--name-status", "-1"), stdout = TRUE),
    system2("git", args = c("remote", "-v"), stdout = TRUE)
  )
}

## session info
sessionInfo()
```

</details>
