...
coverage_milestones <- function(vaccination_coverage) {
  ...
  vaccination_coverage_milestones <-
    ...
    mutate(over_16 = !(age_band_id %in% c("0-11", "12-15")),
    ...
    mutate(eligible_group = str_remove_all(eligible_group, "_milestone")) %>%
    mutate(milestone = case_when(!is.na(milestone) ~ as.character(glue(
      "{eligible_group}_{milestone}"
    )),
    TRUE ~ NA_character_)) %>%
    ...
    ungroup() %>%
    rename(date = time_dose_2) %>%
    select(-eligible_group) 
  
  vaccination_coverage_milestones
  # %>% 
  #   bind_rows(
  #     tibble(
  #       date = max(vaccination_coverage$time_dose_2, na.rm = TRUE),
  #       milestone = "terminal"
  #     )
  #   )
  #   
