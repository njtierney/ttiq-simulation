...
#' Use dist_test_turnaround_time to create cases entering the system then
#' 
sim_tracing <- function(cases_scenario, derived_delay_distributions, n_samples=1e4) {
  ...
  # hardcoded parameters for now
  {
    ...
    samples_test_turnaround_time = generated_samples %>%
      filter(scenario == !!scenario) %>%
      pull(samples_test_turnaround_time) 
    ...
    scenario_samples = scenario_samples %>%
      mutate(samples_time_to_interview = (interview_date - notification_date) %>%
               replace_na(-2))
    }
